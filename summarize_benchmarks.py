...
class BenchmarkSummarizer:
    ...
    def load_benchmark_data(self, model_name: str, run_id: Optional[str] = None) -> Dict[str, List[Dict]]:
                ...
                for json_file in backend_dir.glob("**/*.json"):
                    try:
                        with open(json_file, 'r') as f:
                            ...
                            if len(path_parts) > 3:  # results/model/backend/run_id/file.json
    ...
    def get_performance_ranking(self, data: Dict[str, List[Dict]]) -> Dict[str, List[tuple]]:
        ...
        for metric in ['mean_ttft_ms', 'mean_tpot_ms', 'p99_ttft_ms', 'p99_tpot_ms']:
            rankings[f"Best {metric.replace('_', ' ').replace('ms', '(ms)').title()}"] = sorted(
                metrics.items(), 
                key=lambda x: x[1][metric]
            )
    ...
    def print_summary(self, model_name: str, data: Dict[str, List[Dict]], output_file: Optional[str] = None):
        ...
        for rank_name, ranked_backends in rankings.items():
            ...
            for i, (backend, metrics) in enumerate(ranked_backends, 1):
                metric_key = key_mapping.get(rank_name, rank_name.lower().replace('best ', '').replace(' ', '_').replace('(ms)', '_ms'))
        ...
        for backend_name, backend_data in data.items():
            if len(backend_data) > 1:
                ...
                print(f"**{backend_name}**: {len(backend_data)} runs from {min(dates)} to {max(dates)}")
        ...
        if not df.empty:
            ...
            best_throughput = df.loc[df['Output Throughput (tokens/s)'].astype(float).idxmax(), 'Backend']
            ...
            print(f"- **Throughput range**: {throughput_values.min():.1f} - {throughput_values.max():.1f} tokens/s")
...
def main():
    parser = argparse.ArgumentParser(description='Summarize benchmark results for a given model')
