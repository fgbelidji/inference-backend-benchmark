...
def main():
    ...
    for name, spec in backends.items():
        ...
        # start container
        run(f"docker run --rm -d {GPU_FLAG} -p {port}:{port} --shm-size 32G --name {name} {image} {cmdline}")
    ...
    print(f"Use: python summarize_benchmarks.py {model.replace('/', '-')} to view summary")
